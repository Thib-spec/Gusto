...
export default function CategorieDropDownComponent(props) {
  ...
  function handleUpdateCategorie() {
    try {
      let a = document.getElementById(
        "descriptionCategorie" + props.categorie.id
      ).value;
      axios
        .put(
          "http://api.gustosolutions.fr/api/category/" +
            props.categorie.id_category,
          {
  ...
  function handleDeleteCatégorie() {
    try {
      axios
        .delete(
          "http://api.gustosolutions.fr:3001/api/category/" +
            props.categorie.id_category
        )
        .then((res) => {
          console.log(res);
  ...
  return (
    <div className="categories-list-element">
      {open === true ? (
        <div>
          <div
            className="categories-list-element-title-fold"
            onClick={() => handleOpen()}
          >
          ...
          <div className="categories-list-element-sub">
            <div className="categories-list-element-sub-description">
              ...
              <textarea
                type="text"
                className=" categories-list-element-sub-description-input"
                id={"descriptionCategorie" + props.categorie.id}
                placeholder="Description de la catégorie"
              >
                {props.categorie.description}
              </textarea>
            </div>
            <div className="categories-list-element-sub-buttons ">
              <button
                type="button"
                className="categories-list-element-sub-buttons-element btn btn-info"
              ...
              <button
                type="button"
                className="categories-list-element-sub-buttons-element btn btn-danger"
                onClick={() => handleDeleteCatégorie()}
              >
                Supprimer
              </button>
            </div>
